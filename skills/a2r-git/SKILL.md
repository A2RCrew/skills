...
con que correo se commitea y si el push pasa pruebas y build, que no todos los
...
Los tipos y el lint ya los ha comprobado el hook en cada commit, y con las reglas
que llevan nuestros repos eso adelanta casi todo lo que rompe el build; casi,
porque el lint no compila ni ejecuta nada. Lo que falta son las pruebas y el
build: donde importe, `/a2r-git:init` los deja en el hook de pre-push, y donde
no, lanza a mano `pnpm test` y `pnpm build` si el cambio toca tipos,
configuracion o dependencias. Si algo falla, no subas: arreglalo o deja el commit
en local y dilo.

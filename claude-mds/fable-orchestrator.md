...
- If computer use is helpful for completing or verifying work, shell out to gpt-5.6 via the codex-computer-use skill.
...
- gpt-5.6 is only reachable through the Codex CLI – `codex exec` / `codex review` (my ~/.codex/config.toml defaults to gpt-5.6 at xhigh reasoning). Use the codex-implementation, codex-review, and codex-computer-use skills; for work they don't cover (investigation, data analysis), run `codex exec -s read-only` directly with a self-contained prompt.
...
- Codex runs can exceed Bash's default timeout: pass an explicit timeout, or run in the background and poll for the report file.
